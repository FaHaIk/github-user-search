<template>
  <div>
    <div v-if="left == false" class="card" @mouseover="mOver" @mouseleave="mLeave">
      <div class="rest-wrapper">
        ...
        <div class="ico-wrapper">
          ...
          <div class="float icon-text">{{ stars }}</div>
          <div class="puffer"></div>
          <div class="float" v-html="forkIcon"></div>
          <div class="float icon-text">{{ forks }}</div>
          <div class="puffer"></div>
          <div class="float" v-html="codeIcon"></div>
          <div class="float icon-text">{{ language }}</div>
        </div>
      </div>
      <div class="divider" :class="{'bgDiv': !hover, 'bgDivH': hover }"></div>
      <div class="date-wrapper-right">
        <span class="date-txt">{{ month }} {{ day }}, {{ year }}</span>
      </div>
    </div>

    <div v-else class="card" @mouseover="mOver" @mouseleave="mLeave">
      <div class="date-wrapper">
        <span class="date-txt">{{ month }} {{ day }}, {{ year }}</span>
      </div>
      <div class="divider" :class="{'bgDiv': !hover, 'bgDivH': hover }"></div>
      <div class="rest-wrapper-right">
        <div class="name">
          <a :href="url" target="_blank">{{ name }}</a>
        </div>
        <div class="desc">{{ description }}</div>
        <div class="ico-wrapper">
          <div class="float-left" v-html="starIcon"></div>
          <div class="float-left icon-text">{{ stars }}</div>
          <div class="puffer-left"></div>
          <div class="float-left" v-html="forkIcon"></div>
          <div class="float-left icon-text">{{ forks }}</div>
          <div class="puffer-left"></div>
          <div class="float-left" v-html="codeIcon"></div>
          <div class="float-left icon-text">{{ language }}</div>
        </div>
      </div>
    </div>
  </div>
